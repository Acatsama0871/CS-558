...
== 1.2 CLI Functionalities
The program is written as a command line tool. The usage can be found by running `python main.py --help`. All results can be replicated by running the `run.sh` script. The entry point is `python run.py`.
  
== 1.2.1 keypoint corner detection: 
    - Entry point: #underline(text(blue)[python run.py corner-keypoint])
    #align(center + top)[
    #figure(
  image("images/corner.png", alt: "Corner Keypoint",),
  caption: [
    Corner Keypoint
  ],
)
...
== 1.2.2 RANSAC Line Fitting: 
    - Entry point: #underline(text(blue)[python run.py ransac-linefit])
    #align(center + top)[
    #figure(
  image("images/ransac.png", alt: "ransac"),
  caption: [
    RANSAC
  ],
)
...
- Entry point: #underline(text(blue)[python run.py hough-transform])
    #align(center + top)[
    #figure(
  image("images/hough.png", alt: "though"),
  caption: [
    Hough Transform
  ],
)
...
= 2. Results
== 2.1 Corner Keypoints
#align(center + top)[
    #figure(
  image("images/keypoint.png", alt: "though", width: 80%),
  caption: [
    Corner Keypoints; \u{03C3} = 6.0, threshold = 16
  ],
)
]

== 2.2 RANSAC Line Fitting
#v(3%)
...
#table(
  ...
  align: horizon,
  [*Parameters*], [*Values*],
  [number of lines to detect], [4],
  [total number of iterations], [4950],
  [inlier threshold], [2.0]
)
]

#align(center + top)[
    #figure(
  image("images/ransac_result.png", alt: "though", width: 80%),
  caption: [
    RANSAC Line Fitting Results; 
    With numbers of inliers supporting the line in red
  ],
)
]
...
To avoid detecting the lines that are too close to each other, we add an extra "layer" around the algorithm to remove inliers once the line with the highest votes has been detected before detecting the next line.

#algo(
  title: "Hough TransFrom Remove Inliers",
  parameters: ("keypoints set", "num_lines", "min_dist")
)[
  lines = []\
  for i in 1...num_lines:#i\
    cur_line = Hough_Transform(keypoint set) \
    lines.append(cur_line)\
    inliers = {points: perpendicular_distance_to_line(keypoints, cur_line) < min_dist} \ 
    keypoints set = keypoints - inliers
]
#v(2%)

== 2.2.2 Results
=== 2.2.2.1 Regular Dimension

#align(center + top)[
    #figure(
  image("images/regular/hough_theta_step_2.0_rho_step_2.0.png", alt: "though", width: 80%),
  caption: [
    Hough Transform Regular Bin Size; \u{03B8} Step: 2.0, \u{03C1} Step: 2.0
  ],
)
]
#v(3%)
#align(center + top)[
    ...
    grid(
        columns: 4,
        gutter: 2mm,
 image("images/regular/hough_accumulator_theta_step_2.0_rho_step_2.0_line_0.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 image("images/regular/hough_accumulator_theta_step_2.0_rho_step_2.0_line_1.png", alt: "though", width: 80%, height: 50%, fit: "contain"),
 image("images/regular/hough_accumulator_theta_step_2.0_rho_step_2.0_line_2.png", alt: "though", width: 80%, height: 50%, fit: "contain"),
 
 image("images/regular/hough_accumulator_theta_step_2.0_rho_step_2.0_line_3.png", alt: "though", width: 80%, height: 50%, fit: "contain"),
    ),
    caption: "Voting Plot For Each Line"
)
]

=== 2.2.2.2 Reduced Dimension

#align(center + top)[
    #figure(
  image("images/reduced/hough_theta_step_1.0_rho_step_1.0.png", alt: "though", width: 80%),
  caption: [
    Hough Transform Reduced Bin Size; \u{03B8} Step: 1.0, \u{03C1} Step: 1.0
  ],
)
]
#v(3%)
#align(center + top)[
    ...
    grid(
        columns: 4,
        gutter: 2mm,
 image("images/reduced/hough_accumulator_theta_step_1.0_rho_step_1.0_line_0.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 image("images/reduced/hough_accumulator_theta_step_1.0_rho_step_1.0_line_1.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 image("images/reduced/hough_accumulator_theta_step_1.0_rho_step_1.0_line_2.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 
 image("images/reduced/hough_accumulator_theta_step_1.0_rho_step_1.0_line_3.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
    ),
    caption: "Voting Plot For Each Line"
)
]
...
=== 2.2.2.3 Doubled Dimension

#align(center + top)[
    #figure(
  image("images/double/hough_theta_step_4.0_rho_step_4.0.png", alt: "though", width: 80%),
  caption: [
    Hough Transform Dobled Bin Size; \u{03B8} Step: 4.0, \u{03C1} Step: 4.0
  ],
)
]
#v(3%)
...
image("images/double/hough_accumulator_theta_step_4.0_rho_step_4.0_line_0.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 image("images/double/hough_accumulator_theta_step_4.0_rho_step_4.0_line_1.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 image("images/double/hough_accumulator_theta_step_4.0_rho_step_4.0_line_2.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
 
 image("images/double/hough_accumulator_theta_step_4.0_rho_step_4.0_line_3.png", alt: "though", width: 80%, height: 50%, fit: "contain"), 
    ),
    caption: "Voting Plot For Each Line"
)
]
